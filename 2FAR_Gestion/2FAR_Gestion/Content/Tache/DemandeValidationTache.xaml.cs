...
namespace _2FAR_Gestion
{
    ...
    public partial class DemandeValidation
    {
        public DemandeValidation()
        {
            ...
            {
                listCartes.Children.Add(new Carte(attendreValidation.dte_demande + " | " + MainWindow.listeTP.Where(Tp => Tp.idTP == attendreValidation.tache.fk_id_tp ).First().nomTP + " | " + MainWindow.listePromotions.Where(promo => promo.idPromo == attendreValidation.utilisateur.fk_id_promo ).First().nomPromo, attendreValidation.tache.descriptionTache + " | " + attendreValidation.utilisateur.nomUtilisateur.Split().First() + " " + attendreValidation.utilisateur.prenomUtilisateur, actionsButton));
            }
        }
        public void valider()
        {
